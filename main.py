...
class MultiFactorAuth(tk.Tk):
  ...
  def __init__(self):
    ...
    self.btn_login = tk.Button(
        self.frame_login, text="LOG IN", command=self.authorize)
  ...
  def authorize(self):
    ...
    if (student_username == self.username and student_password == self.password):
        ...
        self.lbl_auth = tk.Label(
            self.frame_auth, text=self.security_question + "?", bg="plum4")
        ...
        self.btn_auth = tk.Button(
            self.frame_auth, text="AUTHENTICATE", command=self.authenticate)
  ...
  def authenticate(self):
    ...
    if (factor_info == self.answer):
        ...
        lbl_msg = tk.Label(
            self.frame_restrict, text="This is a restricted application.", bg="sienna2")
        lbl_msg.config(font=("Arial", 18))
        lbl_msg.pack(pady=15)

        lbl_auth = tk.Label(
            self.frame_restrict, text=message, bg="sienna2")
        lbl_auth.pack(pady=30)

        # show this restricted app frame
        self.frame_restrict.tkraise()
    else:
        mb.showinfo("Authentication",
                    "We're sorry, but our records do not match your entry.")


base64_message = "UlN7QkBDS1dAUkQkXyROQEszU30="
base64_bytes = base64_message.encode('ascii')
message_bytes = base64.b64decode(base64_bytes)
message = message_bytes.decode('ascii')

base64_message2 = "dGhlYmlnY2hlZXNl"
...
base64_message3 = "SWFic29sdXRlbHlMT1ZFcHVwcGllcyEhIQ=="
...
question = "is RITSEC the best club?"
answer = "yes"
